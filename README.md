...
Модель обучалась на комбинированных источниках: физиологические сигналы BioRadio (ECG/PPG/resp) и видеоданные с последующей скелетизацией. Такое объединение позволяет улавливать как автономные реакции (сердечный ритм, дыхание), так и поведенческие маркеры (микродвижения корпуса, головы, рук), что повышает чувствительность к признакам лжи.

Для извлечения позовых маркеров использована MediaPipe: библиотека обеспечивает детекцию и трекинг тела/лица/рук в реальном времени. Координаты ключевых точек используются для расчета углов, расстояний и их динамики во времени, после чего синхронизируются с окнами ЭЭГ/ЭКГ/дыхания. Это позволяет сравнивать поведенческие реакции и физиологические сигналы в одних и тех же временных интервалах и обучать модель на их совместном представлении.
